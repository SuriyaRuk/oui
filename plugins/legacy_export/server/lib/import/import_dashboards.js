 ...
 * under the License.
 */
async function importDashboards(savedObjectsClient, objects, {
  overwrite,
  exclude
}) {
  ...
  const docs = objects.filter(item => !exclude.includes(item.type)) // filter out any document version, if present
  .map(({
    version,
    ...doc
  }) => ({ ...doc,
  ...
  const results = await savedObjectsClient.bulkCreate(docs, {
    overwrite
  });
  return {
    objects: results.saved_objects
  };
}