 ...
 * under the License.
 */
function splitByTerms(req, panel, opensearchQueryConfig, indexPattern) {
  return next => doc => {
    panel.series.filter(c => c.aggregate_by && c.aggregate_function).forEach(column => {
      ...
      if (column.filter) {
        (0, _helpers.overwrite)(doc, `aggs.pivot.aggs.${column.id}.column_filter.filter`, _server.opensearchQuery.buildOpenSearchQuery(indexPattern, [column.filter], [], opensearchQueryConfig));
      }
    });
    return next(doc);
  };
