...
function ratios(req, panel, opensearchQueryConfig, indexPatternObject) {
  return next => doc => {
    panel.series.forEach(column => {
      ...
      if (column.metrics.some(filter)) {
        column.metrics.filter(filter).forEach(metric => {
          (0, _helpers.overwrite)(doc, `${aggRoot}.timeseries.aggs.${metric.id}-numerator.filter`, _server.opensearchQuery.buildOpenSearchQuery(indexPatternObject, metric.numerator, [], opensearchQueryConfig));
          (0, _helpers.overwrite)(doc, `${aggRoot}.timeseries.aggs.${metric.id}-denominator.filter`, _server.opensearchQuery.buildOpenSearchQuery(indexPatternObject, metric.denominator, [], opensearchQueryConfig));
