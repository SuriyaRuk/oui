 ...
 * under the License.
 */
function query(req, panel, annotation, opensearchQueryConfig, indexPattern, capabilities) {
  return next => doc => {
    ...
    doc.query = _server.opensearchQuery.buildOpenSearchQuery(indexPattern, queries, filters, opensearchQueryConfig);
    const timerange = {
      range: {
        [timeField]: {
    ...
    if (annotation.query_string) {
      doc.query.bool.must.push(_server.opensearchQuery.buildOpenSearchQuery(indexPattern, [annotation.query_string], [], opensearchQueryConfig));
    }

    if (!annotation.ignore_panel_filters && panel.filter) {
      doc.query.bool.must.push(_server.opensearchQuery.buildOpenSearchQuery(indexPattern, [panel.filter], [], opensearchQueryConfig));
    }
