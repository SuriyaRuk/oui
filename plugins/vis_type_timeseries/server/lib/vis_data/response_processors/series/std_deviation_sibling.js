...
function stdDeviationSibling(resp, panel, series, meta) {
  return next => results => {
    ...
    if (metric.mode === 'band' && metric.type === 'std_deviation_bucket') {
      (0, _helpers.getSplits)(resp, panel, series, meta).forEach(split => {
        const data = split.timeseries.buckets.map(bucket => [bucket.key, (0, _helpers.getSiblingAggValue)(split, { ...metric,
          mode: 'upper'
        }), (0, _helpers.getSiblingAggValue)(split, { ...metric,
          mode: 'lower'
        })]);
        results.push({
          id: split.id,
