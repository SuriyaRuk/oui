...
exports.isDeniedURL = isDeniedURL;
...
function isDeniedURL(configuredUrl, deniedIPs) {
  ...
  const isDenied = deniedIPs.some(deniedIP => new _ipCidr.default(deniedIP).contains(ip));
  return isDenied;
}
/**
...
function isValidConfig(deniedIPs, allowedUrls, configuredUrl) {
  if (deniedIPs.length === 0) {
    ...
    if (exports.isDeniedURL(configuredUrl, deniedIPs) || !allowedUrls.includes(configuredUrl)) return false;
  }
