 ...
 */
const getLocalStats = async (clustersDetails, // array of cluster uuid's
...
context // StatsCollectionContext contains logger and version (string)
  ...
  return await Promise.all(clustersDetails.map(async clustersDetail => {
    const [clusterInfo, clusterStats, nodesUsage, opensearchDashboards, dataTelemetry] = await Promise.all([(0, _get_cluster_info.getClusterInfo)(opensearchClient), // cluster info
    (0, _get_cluster_stats.getClusterStats)(opensearchClient), // cluster stats (not to be confused with cluster _state_)
    (0, _get_nodes_usage.getNodesUsage)(opensearchClient), // nodes_usage info
