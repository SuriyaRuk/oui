...
const getTelemetryOptIn = ({
  ...
  const lastOpenSearchDashboardsVersion = telemetrySavedObject.lastVersionChecked; // if the last OpenSearch Dashboards version isn't set, or is somehow not a string, return null

  if (typeof lastOpenSearchDashboardsVersion !== 'string') return null; // if version hasn't changed, just return enabled value
  ...
  const currentSemver = parseSemver(currentOpenSearchDashboardsVersion); // if either version is invalid, return null
