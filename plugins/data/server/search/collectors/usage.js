...
function usageProvider(core, config) {
  const getTracker = eventType => {
    return async duration => {
      var _config$search, _config$search$usageT;

      if (config !== null && config !== void 0 && (_config$search = config.search) !== null && _config$search !== void 0 && (_config$search$usageT = _config$search.usageTelemetry) !== null && _config$search$usageT !== void 0 && _config$search$usageT.enabled) {
        ...
        attributes[eventType]++; // Only track the average duration for successful requests
        ...
        } catch (e) {// Version conflict error, swallow
        }
      }
    };
  };
