...
function readFieldCapsResponse(fieldCapsResponse) {
  ...
  const opensearchDashboardsFormattedCaps = Object.keys(capsByNameThenType).reduce((agg, fieldName) => {
    const capsByType = capsByNameThenType[fieldName];
    const types = Object.keys(capsByType); // If a single type is marked as searchable or aggregatable, all the types are searchable or aggregatable
    ...
    const uniqueOpenSearchDashboardsTypes = (0, _lodash.uniq)(types.map(_common.castOpenSearchToOsdFieldTypeName));
    ...
    if (uniqueOpenSearchDashboardsTypes.length > 1) {
      const field = {
        ...
        conflictDescriptions: types.reduce((acc, opensearchType) => ({ ...acc,
    ...
    const field = {
      name: fieldName,
      type: (0, _common.castOpenSearchToOsdFieldTypeName)(opensearchType),
      esTypes: types,
      searchable: isSearchable,
      aggregatable: isAggregatable,
      readFromDocValues: (0, _should_read_field_from_doc_values.shouldReadFieldFromDocValues)(isAggregatable, opensearchType)
    }; // This is intentionally using a "hash" and a "push" to be highly optimized with very large indexes
  ...
  subFields.forEach(field => {
    ...
    const parentFieldCaps = parentFieldNames.map(parentFieldName => opensearchDashboardsFormattedCaps.hash[parentFieldName]);
    const parentFieldCapsAscending = parentFieldCaps.reverse();

    if (parentFieldCaps && parentFieldCaps.length > 0) {
      let subType = {}; // If the parent field is not an object or nested field the child must be a multi field.
