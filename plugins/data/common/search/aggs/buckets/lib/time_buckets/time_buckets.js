...
var _calc_opensearch_interval = require("./calc_opensearch_interval");
...
class TimeBuckets {
  ...
  getInterval(useNormalizedOpenSearchInterval = true) {
    ...
    const decorateInterval = interval => {
      const opensearchInterval = useNormalizedOpenSearchInterval ? (0, _calc_opensearch_interval.convertDurationToNormalizedOpenSearchInterval)(interval) : (0, _calc_opensearch_interval.convertIntervalToOpenSearchInterval)(String(this._originalInterval));

      const prettyUnits = _moment.default.normalizeUnits(opensearchInterval.unit);

      return Object.assign(interval, {
        description: opensearchInterval.value === 1 ? prettyUnits : opensearchInterval.value + ' ' + prettyUnits + 's',
