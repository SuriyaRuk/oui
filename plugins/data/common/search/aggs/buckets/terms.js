 ...
 * under the License.
 */
const termsAggFilter = ['!top_hits', '!percentiles', '!median', '!std_dev', '!derivative', '!moving_avg', '!serial_diff', '!cumulative_sum', '!avg_bucket', '!max_bucket', '!min_bucket', '!sum_bucket'];
...
const getTermsBucketAgg = () => new _bucket_agg_type.BucketAggType({
  ...
  params: [{
    ...
    write(agg, output, aggs) {
      ...
      let orderAgg = agg.params.orderAgg || aggs.getResponseAggById(agg.params.orderBy); // TODO: This works around an OpenSearch bug the always casts terms agg scripts to strings
