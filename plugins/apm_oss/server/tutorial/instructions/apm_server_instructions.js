...
const createEditConfig = () => ({
  title: _i18n.i18n.translate('apmOss.tutorial.editConfig.title', {
    defaultMessage: 'Edit the configuration'
  }),
  textPre: _i18n.i18n.translate('apmOss.tutorial.editConfig.textPre', {
    defaultMessage: "If you're using a secured version of OpenSearch, you must specify \
credentials in the `apm-server.yml` config file."
  }),
  commands: ['output.opensearch:', '    hosts: ["<opensearch_url>"]', '    username: <username>', '    password: <password>']
});

exports.createEditConfig = createEditConfig;

const createStartServer = () => ({
  title: _i18n.i18n.translate('apmOss.tutorial.startServer.title', {
    defaultMessage: 'Start APM Server'
  }),
  textPre: _i18n.i18n.translate('apmOss.tutorial.startServer.textPre', {
    defaultMessage: 'The server processes and stores application performance metrics in OpenSearch.'
  })
});
...
const createDownloadServerTitle = () => _i18n.i18n.translate('apmOss.tutorial.downloadServer.title', {
  defaultMessage: 'Download and unpack APM Server'
});

const createDownloadServerOsx = () => ({
  title: createDownloadServerTitle(),
  commands: ['curl -L -O https://artifacts.opensearch.org/downloads/apm-server/apm-server-{config.opensearchDashboards.version}-darwin-x64.tar.gz', 'tar xzvf apm-server-{config.opensearchDashboards.version}-darwin-x64.tar.gz', 'cd apm-server-{config.opensearchDashboards.version}-darwin-x64/']
});
...
const createDownloadServerDeb = () => ({
  title: createDownloadServerTitle(),
  commands: ['curl -L -O https://artifacts.opensearch.org/downloads/apm-server/apm-server-{config.opensearchDashboards.version}-amd64.deb', 'sudo dpkg -i apm-server-{config.opensearchDashboards.version}-amd64.deb'],
  textPost: _i18n.i18n.translate('apmOss.tutorial.downloadServerTitle', {
...
const createDownloadServerRpm = () => ({
  title: createDownloadServerTitle(),
  commands: ['curl -L -O https://artifacts.opensearch.org/downloads/apm-server/apm-server-{config.opensearchDashboards.version}-x64.rpm', 'sudo rpm -vi apm-server-{config.opensearchDashboards.version}-x64.rpm'],
  textPost: _i18n.i18n.translate('apmOss.tutorial.downloadServerRpm', {
    defaultMessage: 'Looking for the 32-bit packages? See the [Download page]({downloadPageLink}).',
    values: {
      downloadPageLink: '{config.docs.base_url}downloads/apm/apm-server'
    }
...
function createWindowsServerInstructions() {
  ...
  return [{
    title: createDownloadServerTitle(),
    textPre: _i18n.i18n.translate('apmOss.tutorial.windowsServerInstructions.textPre', {
...
PowerShell.\n5. From the PowerShell prompt, run the following commands to install APM Server as a Windows service:',
    ...
    textPost: _i18n.i18n.translate('apmOss.tutorial.windowsServerInstructions.textPost', {
