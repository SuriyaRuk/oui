 ...
 * under the License.
 */
const LATEST_PLUGIN_BASE_URL = 'https://ci.opensearch.org/ci/dbc/distribution-build-opensearch-dashboards';
...
function generatePluginUrl(version, plugin) {
  const [platform, type] = process.platform === 'win32' ? ['windows', 'zip'] : [process.platform, 'tar'];
...
function parse(command, options, osdPackage) {
  const settings = {
