...
function getLegacyRawConfig(config, pathConfig) {
  const rawConfig = config.toRaw(); // OpenSearch config is solely handled by the core and legacy platform
...
class LegacyService {
  ...
  async start(startDeps) {
    ...
    this.log.debug('starting legacy service'); // Receive initial config and create osdServer/ClusterManager.
