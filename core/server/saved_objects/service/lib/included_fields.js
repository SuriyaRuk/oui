...
function includedFields(type = '*', fields) {
  ...
  }, []).concat('namespace').concat('namespaces').concat('type').concat('references').concat('migrationVersion').concat('updated_at').concat('originId').concat(fields); // v5 compatibility
}