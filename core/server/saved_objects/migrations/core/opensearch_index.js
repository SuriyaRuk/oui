...
async function fetchInfo(client, index) {
  ...
  const [indexName, indexInfo] = Object.entries(body)[0]; // @ts-expect-error @opensearch-project/opensearch IndexState.alias and IndexState.mappings should be required
...
async function migrationsUpToDate(client, index, migrationVersion, retryCount = 10) {
  try {
...
function assertIsSupportedIndex(indexInfo) {
  ...
  if (!isV7Index) {
    throw new Error(`Index ${indexInfo.indexName} belongs to a version of OpenSearch Dashboards ` + `that cannot be automatically migrated. Reset it.`);
  }
...
function assertResponseIncludeAllShards({
  ...
  if (failed > 0) {
    throw new Error(`Re-index failed :: ${failed} of ${_shards.total} shards failed. ` + `Check OpenSearch cluster health for more information.`);
  }
}
/**
